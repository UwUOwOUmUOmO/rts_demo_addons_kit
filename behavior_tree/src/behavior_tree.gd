...
export(bool) var is_active: bool = false setget set_state
...
func _ready() -> void:
	...
	deps_check()
	while not is_instance_valid(agent):
...
func deps_check():
	if not is_instance_valid(agent):
		agent = get_node_or_null(_agent)
	if not is_instance_valid(blackboard):
		blackboard = get_node_or_null(_blackboard)
