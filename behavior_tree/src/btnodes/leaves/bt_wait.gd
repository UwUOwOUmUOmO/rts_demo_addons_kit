...
# Waits for wait_time seconds, then succeeds. time_in_blackboard is the key where,
# if desired, another amount can be specified. In that case, wait_time is overridden.


export(float) var wait_time: float
export(String) var time_in_blackboard: String
...
func _tick(agent: Node, blackboard: Blackboard) -> bool:
	if time_in_blackboard:
		wait_time = blackboard.get_data(time_in_blackboard)
